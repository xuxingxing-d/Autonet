...
class _Attention(tf.keras.layers.Layer):
    ...
    def __init__(self, data_format='channels_last', **kwargs):
        ...
        self.data_format = data_format  #它可能是用于实现注意力机制的某个模块或组件
...
class SelfAttnModel(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        return self.attn([q, k, v, inputs])#自注意力

def downsample(filters, size, shape, apply_batchnorm=True, attn_layer=False):#编码器的下采样
...
def upsample(filters, size, shape, apply_dropout=False, attn_layer=False):#解码器的上采样
    initializer = tf.random_normal_initializer(0., 0.02)
